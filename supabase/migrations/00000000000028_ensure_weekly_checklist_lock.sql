-- ensure_weekly_checklist's "if not exists then insert" is racy: two
-- concurrent calls for the same brand-new week (e.g. React StrictMode's
-- double-invoked effect, or two users opening a new week at once) can both
-- pass the not-exists check before either commits, and the second's insert
-- then violates weekly_checklist_items_week_sort_idx. An advisory lock
-- keyed by week_start serializes calls for the same week without affecting
-- different weeks.
