-- Every shift is meant to run with exactly 3 bartenders. Auto-generated
-- shifts previously left required_staff_count null, so the understaffing
-- indicators built around this column (ShiftDetail, MyShifts, Shifts,
-- and now the dashboard's upcoming-shifts list) never had anything to
-- compare against for shifts nobody had edited by hand.
...
begin
  ...
  for i in 0..12 loop
    ...
    values
      (v_thu_start, v_thu_mid, 'opening', 'published', v_week, auth.uid(), 3),
      (v_thu_mid, v_thu_end, 'closing', 'published', v_week, auth.uid(), 3)
