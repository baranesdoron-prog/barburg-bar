-- finish_shift_closing() only ever allowed the specifically-assigned
-- shift manager (by employee_id match) or an administrator or the dead
-- 'bar_manager' enum role -- never widened to the real 'shift_manager'
-- role when migration 48 widened everything else. Any bar manager should
-- be able to close a shift, not just whoever was assigned to it.
