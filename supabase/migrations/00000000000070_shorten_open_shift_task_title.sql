-- Shorten the item-9 weekly checklist task title.
update weekly_checklist_items
set title = 'פתיחת משמרת וסידור אנשים'
where title = 'פתיחת משמרת הפתיחה (19:30–22:00) ומשמרת הסגירה (22:00–00:30) במסך המשמרות ושיבוץ ברמנים לכל אחת מהן; עדכון הצוות בוואטסאפ אם חסר כוח אדם';
...
begin
  ...
  if not exists (select 1 from weekly_checklist_items where week_start = p_week_start) then
    ...
    insert into weekly_checklist_items (week_start, sort_order, title, due_date, is_optional) values
      ...
      (p_week_start, 9, 'פתיחת משמרת וסידור אנשים', p_week_start + 3, false),
